...
class CassieEnv(MujocoEnv):
    ...
    def __init__(self, config, **kwargs):
        ...
        self._healthy_z_range = config.get("healthy_z_range", (0.40, 2.0))
    ...
    def is_healthy(self):
        ...
        is_healthy = min_z < self.data.qpos[2] < max_z and (not self.contact or self.data.xpos[c.LEFT_FOOT, 2] < 0.15 or self.data.xpos[c.RIGHT_FOOT, 2] < 0.15) # and (self.contact == False or  self.data.geom_xpos[-2:,2][0] < 0.15 or self.data.geom_xpos[-2:,2][1] < 0.15)
        #and self.steps <= c.MAX_STEPS # and (self.contact == False or  self.data.geom_xpos[-2:,2][0] < 0.15 or self.data.geom_xpos[-2:,2][1] < 0.15)
    ...
    def _get_obs(self):
        ...
        for key in c.sensor_ranges.keys():
            for x in self.data.sensor(key).data:
                temp.append((x-c.obs_ranges[0][i])/(c.obs_ranges[1][i]-c.obs_ranges[0][i]))
                i+=1
    ...
    def compute_reward(self, action):
        ...
        if( c.left_foot_force_idx in contacts):
            m.mj_contactForce(self.model, self.data, contacts.index(c.left_foot_force_idx), contact_force_left_foot)
        if(c.right_foot_force_idx in contacts):
            m.mj_contactForce(self.model, self.data, contacts.index(c.right_foot_force_idx), contact_force_right_foot)
        
        # contact_force_right_foot =self.data.cfrc_ext[c.right_foot_force_idx]
        # contact_force_left_foot = self.data.cfrc_ext[c.left_foot_force_idx]
        # # Feet Contact Forces
        # contact_force_right_foot = np.zeros(6)
        ...
        if self.data.xpos[c.LEFT_FOOT, 2] < 0.12 or self.data.xpos[c.RIGHT_FOOT, 2] < 0.12:
            self.contact  = True
