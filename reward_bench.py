...
class RewardCassieMonitor:
    def __init__(self, max_data_points=300):
        ...
        self.observation, self.reset_info = self.env.reset() # Store reset_info if needed later
        self.timestep = 0
    ...
    def setup_figure(self):
        ...
        self.gs = GridSpec(
            4, 4, figure=self.fig, width_ratios=[1.5, 1, 1, 1], height_ratios=[1, 1, 1, 1]
        )
        ...
        self.ax_heights = self.fig.add_subplot(self.gs[0, 3]) # Keep heights for context
        ...
        self.ax_norm_all = self.fig.add_subplot(self.gs[3, :]) # Span all columns in row 3
        ...
        self.lines = {}
        self.all_axes = [
            self.ax_raw_vel, self.ax_raw_orient_accel, self.ax_heights,
            self.ax_raw_forces, self.ax_raw_speeds, self.ax_phase_coeffs,
            self.ax_exp_quantities, self.ax_rewards_comp, self.ax_total_reward,
            self.ax_norm_all,
        ]
        ...
        self._create_lines(self.ax_raw_vel, raw_vel_keys, sns.color_palette("viridis", 2), labels=["q_vx", "q_vy"])

        # Raw Orient & Accel (gs[0, 2])
        raw_orient_accel_keys = ["raw_q_orientation", "raw_q_pelvis_acc"]
        self._create_lines(self.ax_raw_orient_accel, raw_orient_accel_keys, sns.color_palette("plasma", 2), labels=["q_orient", "q_pelvis_acc"])

        # Heights & RPY (gs[0, 3]) - Context
        height_keys = ["pelvis_height", "left_foot_height", "right_foot_height", "pelvis_roll", "pelvis_pitch", "pelvis_yaw"]
        height_labels = ["Pelvis", "L Foot", "R Foot", "Roll", "Pitch", "Yaw"]
        self._create_lines(self.ax_heights, height_keys, sns.color_palette("Paired", 6), labels=height_labels)


        # Raw Forces (gs[1, 1])
        raw_force_keys = ["raw_q_left_frc", "raw_q_right_frc"]
        self._create_lines(self.ax_raw_forces, raw_force_keys, sns.color_palette("coolwarm", 2), labels=["q_L_frc", "q_R_frc"])

        # Raw Speeds (gs[1, 2])
        raw_speed_keys = ["raw_q_left_spd", "raw_q_right_spd"]
        self._create_lines(self.ax_raw_speeds, raw_speed_keys, sns.color_palette("PRGn", 2), labels=["q_L_spd", "q_R_spd"])

        # Phase Coeffs (gs[1, 3])
        phase_coeff_keys = ["c_frc_left", "c_frc_right", "c_spd_left", "c_spd_right"]
        phase_labels = ["c_frc_L", "c_frc_R", "c_spd_L", "c_spd_R"]
        self._create_lines(self.ax_phase_coeffs, phase_coeff_keys, sns.color_palette("Spectral", 4), labels=phase_labels)


        # Exp Quantities (gs[2, 1]) - Plot a selection
        exp_keys_plot = [
            "exp_q_vx", "exp_q_vy", "exp_q_left_frc", "exp_q_right_frc",
            "exp_q_action", "exp_q_orientation", "exp_q_pelvis_acc", "exp_q_torque"
        ]
        exp_labels = ["exp_vx", "exp_vy", "exp_L_frc", "exp_R_frc", "exp_action", "exp_orient", "exp_pelvis_acc", "exp_torque"]
        self._create_lines(self.ax_exp_quantities, exp_keys_plot, sns.color_palette("tab10", 8), labels=exp_labels)


        # Reward Components (gs[2, 2])
        reward_comp_keys = ["r_biped", "r_cmd", "r_smooth"]
        self._create_lines(self.ax_rewards_comp, reward_comp_keys, sns.color_palette("Accent", 3), labels=["Biped", "Cmd", "Smooth"])


        # Total Reward (gs[2, 3])
        self._create_lines(self.ax_total_reward, ["total_reward"], sns.color_palette("Set1", 1), labels=["Total"])

        # All Normalized Quantities (gs[3, :]) - Combined Plot
        norm_keys = [
            "norm_q_vx", "norm_q_vy",
            "norm_q_orientation", "norm_q_pelvis_acc",
            "norm_q_left_frc", "norm_q_right_frc",
            "norm_q_left_spd", "norm_q_right_spd",
            "norm_q_action", "norm_q_torque" # Added action and torque if available
        ]
        ...
        norm_labels = [
            "vx", "vy", "orient", "pelvis_acc", "L_frc", "R_frc", "L_spd", "R_spd", "action", "torque"
        ]
        # Filter labels corresponding to present keys
        norm_labels_present = [norm_labels[i] for i, k in enumerate(norm_keys) if k in norm_keys_present]

        # Use a larger, distinct color palette
        num_norm_lines = len(norm_keys_present)
        norm_colors = sns.color_palette("tab10", num_norm_lines) # Or "husl", "viridis", etc.

        self._create_lines(self.ax_norm_all, norm_keys_present, norm_colors, labels=norm_labels_present)
    ...
    def _create_lines(self, ax, keys, colors, labels=None, key_suffix=False):
        ...
        if labels is None:
            ...
            labels = [k.replace('raw_', '').replace('norm_', '').replace('exp_', '').replace('_', ' ').title() for k in keys]
        elif len(labels) != len(keys):
            raise ValueError(f"Number of labels ({len(labels)}) must match number of keys ({len(keys)}) for axis {ax.get_title()}")
    ...
    def update_data(self):
        ...
        self.observation, reward, self.terminated, self.truncated, info = self.env.step(action)
        ...
        pelvis_rpy = self.env.quat_to_rpy(pelvis_quaternion, radians=False) # Degrees for plotting
        ...
        if "other_metrics" in info:
                ...
                for key, val in metrics["raw_quantities"].items():
                    ...
                    if data_key in self.data:
                        ...
                        if not hasattr(self, f"_warned_{data_key}"):
                           print(f"Warning: Metric key '{data_key}' not found in self.data")
            ...
            print(f"Warning: 'other_metrics' not found in info dict at timestep {self.timestep}")
        ...
        idx_left = int(round(((phi + THETA_LEFT) % 1.0) * steps_per_cycle)) % steps_per_cycle
        idx_right = int(round(((phi + THETA_RIGHT) % 1.0) * steps_per_cycle)) % steps_per_cycle
        ...
        for key, line in self.lines.items():
            if key in self.data and len(self.data[key]) == len(timesteps):
                ...
                artists.append(line)
                 ...
                 print(f"Warning: Length mismatch for key '{key}'. Timesteps: {len(timesteps)}, Data: {len(self.data[key])}")
                 # Simple padding:
                 padded_data = ([np.nan] * (len(timesteps) - len(self.data[key]))) + self.data[key]
...
if __name__ == "__main__":
    ...
    monitor.run(frames=100, interval=40) # Run indefinitely (or until env terminates/truncates)